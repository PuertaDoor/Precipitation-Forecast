...
from model.cam_unet import CamUNet
from model.vit import VisionTransformer
from model.cam_vit import CAMViT
from model.conv_gru import ConvGRU
from model.cam_swinunet import SwinUnet
...
def set_model(sample, device, args,
    ...
    if args.model == 'unet':
    ...
    elif args.model == 'cam_unet':
        model = CamUNet(input_data=args.input_data,
                     window_size=args.window_size,
                     embedding_dim=args.embedding_dim,
                     n_channels=sample.shape[0] * sample.shape[1],
                     n_classes=args.num_classes,
        ...
        model = VisionTransformer(input_data=args.input_data,
                                  patch_size=5,
    ...
    elif args.model == 'cam_vit':
        model = CAMViT(input_data=args.input_data,
                                  patch_size=5,
                                  in_channels=sample.shape[1],
                                  num_classes=args.num_classes,
                                  emb_size=768,
        ...
        model = ConvGRU(input_data=args.input_data,
                         window_size=args.window_size,
                         input_dim=sample.shape[1],
                         hidden_dim=args.hidden_dim,
                         kernel_size=tuple(args.kernel_size),  # hotfix: only supports single tuple of size 2
        ...
        model = SwinUnet(input_data=args.input_data)
