...
def compute_evaluation_metrics(df: pd.DataFrame):
    """
    Compute evaluation metrics in-place for dataframe containing binary confusion matrix. miss = FN
    """
    ...
    df['hss'] = (2 * (df['hit'] * df['cn'] - df['miss'] * df['fa'])) / (df['fa']*df['fa'] + df['miss']*df['miss'] + 2 * df['hit'] * df['cn'] + (df['fa'] + df['cn']) * (df['hit'] + df['miss']))
    return df
