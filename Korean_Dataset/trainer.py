    ...
    def _epoch(self, data_loader, mode):
        ...
        for i, (images, target, t) in enumerate(pbar):
            ...
            if self.dice_criterion !=None:
                ...
                output_mse = torch.sum(output, dim=1).type(torch.FloatTensor).to(self.device) # reshape from B x C x W x H to B x W x H
                target_mse = target.type(torch.FloatTensor).to(self.device)
                loss += 100 * mse(output_mse, target_mse)
